...
from models.modules.wtconv import WTConv
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if 'scales' in self.yaml:
            scale = self.yaml.get('scale', None)
            if scale is None:
                scale = next(iter(self.yaml['scales']))
                LOGGER.warning(f"scale not set, use scale='{scale}'")
            if scale not in self.yaml['scales']:
                raise KeyError(f"scale '{scale}' not found in scales: {list(self.yaml['scales'].keys())}")
            gd, gw, max_channels = self.yaml['scales'][scale]
            self.yaml['depth_multiple'] = gd
            self.yaml['width_multiple'] = gw
            self.yaml['max_channels'] = max_channels
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, DConv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C2f, C3TR, C3STR, C3SPP, C3Ghost, ASPP, CBAM, nn.ConvTranspose2d, 
                 WTConv, C3k2, A2C2f, C3k2PPA]:
            c1, c2 = ch[f], args[0]
            if c2 != no:
                c2 = make_divisible(min(c2, max_channels) * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C2f, C3STR, C3Ghost]:
